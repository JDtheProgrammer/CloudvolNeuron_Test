...
# ===================================================================================================
# STEP 8 — SPEARMAN-CORRELATION METRIC
# ===================================================================================================

def compute_spearman_correlations(v_dict, reference_key, all_keys):
    """
    Compute Spearman r for every segment relative to the stimulated segment.
    The correlation is computed on first differences so similarity reflects
    temporal waveform shape rather than absolute voltage offset.
    """
    ...
    for key in all_keys:
        ...
        x = v_dict[key]
        r, _ = spearmanr(np.diff(ref), np.diff(x))
        corr_values[key] = 0.0 if np.isnan(r) else float(np.clip(r, -1.0, 1.0))
...
def plot_ttm_fwhm_vs_distance(axes, ttm, fwhm, dist_map, seg_key_to_level,
    ...
    # --- Rescale to actual correlation range ---
    vmin = min(corr_values.values())
    vmax = max(corr_values.values())

    # Optional: tighten range slightly for better contrast
    vmin = max(0.95, vmin)   # clamp lower bound
...
# Compute metrics
corr1 = compute_spearman_correlations(v_dict=v1, reference_key=stim_key,
                                      all_keys=all_segment_keys)
corr2 = compute_spearman_correlations(v_dict=v2, reference_key=stim_key,
                                      all_keys=all_segment_keys)
...
print(f"\nSpearman correlation computed for {len(corr1)} segments (burst run)")
print(f"Spearman correlation computed for {len(corr2)} segments (sustained run)")
...
# ===================================================================================================
# RESULT 1/2 — MORPHOLOGY SPEARMAN-GRADIENT PLOT
# ===================================================================================================

def plot_morphology_gradient(nodes, node_order, node_to_segment, corr_values,
                              title="Morphology — Spearman-correlation gradient"):
    """Plot morphology coloured by Spearman correlation relative to the stim segment."""
    fig, ax = plt.subplots(figsize=FIG_GRADIENT)

    norm = mcolors.Normalize(vmin=-1.0, vmax=1.0)
    # White-to-red colormap for Spearman values: clear/white at -1, deep red at +1
    ...
    #bar.set_label("Spearman correlation (zoomed 0.95–1.0)")
    cbar.set_label("Spearman correlation vs stim site", fontsize=FONT_CBAR)
...
plot_morphology_gradient(nodes, node_order, node_to_segment, corr1,
                         title="Morphology — Spearman-correlation gradient (Burst 1 ms)")
plot_morphology_gradient(nodes, node_order, node_to_segment, corr2,
                         title="Morphology — Spearman-correlation gradient (Sustained 1 s)")
...
def plot_cluster_similarity_distance_panel(
    ...
    if k <= 0:
        ...
        ax.set_ylabel("Spearman correlation vs stimulated segment")
        ax.grid(True, alpha=0.25)
        return
    ...
    ax.set_ylabel("Spearman correlation vs stimulated segment", fontsize=FONT_LABEL)
    ax.set_title(title, fontsize=FONT_TITLE, fontweight='bold')
    ax.tick_params(labelsize=FONT_TICK)
...
def print_cluster_table(att_values, cluster_ids, all_keys,
    ...
    print(f"  {'Rank':>4}  {'Segment label':<35}  {'Class':<9}  "
          f"{'Spearman r':>10}  {'Cluster':>10}  {'Dist (um)':>10}")
    print("-" * 110)
...
print_cluster_table(corr1, clust1, all_segment_keys, soma_index, non_soma_index,
                    ...
                    title="Spearman-correlation / cluster table — 1 ms stimulus")
print_cluster_table(corr2, clust2, all_segment_keys, soma_index, non_soma_index,
                    segment_labels, segment_distance_from_stim,
                    title="Spearman-correlation / cluster table — 1 s stimulus")
...
# Plot Spearman correlation by branch order.
...
def plot_branch_correlation_dotplot(corr_values, cluster_ids, cluster_colors,
    ...
    """
    Dot plot:
      y = Spearman correlation vs stimulated segment
    ...
    for lvl in levels_present:
        ...
        # ruler ticks/labels
        for frac_tick, d_tick in zip([0.0, 0.5, 1.0], [0.0, 0.5 * dmax, dmax]):
            xt = x_left + frac_tick * (x_right - x_left)

            ax.plot([xt, xt], [tick_y0, tick_y1],
                    color='black', lw=0.9, zorder=1, clip_on=False)

            label_txt = f"{d_tick:.0f}" if frac_tick < 1.0 else f"{d_tick:.0f} µm"
            ax.text(
        ...
        # plot every segment in this branch-order group
        for key, d in zip(keys_lvl, dvals):
            ...
            # Keep soma/non-axon dots colored by cluster.
            # Make axon dots neutral gray only.
            if key in soma_index:
                marker = 'D'
    ...
    ax.set_ylabel("Spearman correlation vs stimulated segment", fontsize=FONT_LABEL + 2)
    ...
    ax.legend(
        handles=[stim_handle, soma_handle, distance_handle],
        loc=legend_loc,
        fontsize=FONT_LEGEND,
        framealpha=0.55,
        facecolor='white',
...
plot_branch_correlation_dotplot(
    ...
    "Spearman correlation by Branch Order — 1 ms stimulus",
    v_dict=v1,
...
plot_branch_correlation_dotplot(
    ...
    "Spearman correlation by Branch Order — 1 s stimulus",
    v_dict=v2,
    legend_loc='lower right'
)
...
def plot_branch_correlation_dotplot(corr_values, cluster_ids, cluster_colors,
    ...
    for lvl in levels_present:
        ...
        # Keep the regular non-axon dots colored by cluster.
        # Only the axon dots are made neutral so the annotation carries the axon identity.
        if key in soma_index:
            marker = 'D'
            color  = cluster_colors.get(c, '#888888') if c is not None else '#888888'
            size   = MS_DOT_BRANCH
            ...
            marker = 's'
            color  = '#666666'   # neutral gray for axon dots only
            size   = MS_DOT_BRANCH
        else:
            marker = 'o'
            color  = cluster_colors.get(c, '#cccccc') if c is not None else '#cccccc'
    ...
    # Legend — no cluster entries, since this Spearman version focuses on spatial layout and trace similarity.
    stim_handle = plt.Line2D([0], [0], marker='*', color='w',
                             markerfacecolor='black', markersize=18, label='Stimulated')
    soma_handle = plt.Line2D([0], [0], marker='D', color='w',
                             markerfacecolor='grey', markersize=13, label='Soma segment')
    ...
    ax.legend(
        handles=[stim_handle, soma_handle, axon_handle, distance_handle],
        loc=legend_loc,
        fontsize=FONT_LEGEND,
        framealpha=0.55,
        facecolor='white',
